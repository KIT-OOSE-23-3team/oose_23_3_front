...
function Rental() {
  ...
  const init = () => {
    axios.get("https://119.56.139.127:5050/rentalOfficeFindAll").then((r) => {
  ...
  const getBicycle = (office) => {
    axios.get(`https://119.56.139.127:5050/bicycleSearch/${office}`).then((r) => {
  ...
  const rental = () => {
    ...
    axios.get(`https://119.56.139.127:5050/bicycleRental/${bicycleNum}`, { withCredentials: true }).then((r) => {
      alert("대여 성공!");
